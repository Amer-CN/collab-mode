...
**开发窗口不限**（2026-09-13 定）：插件/面板这类 DSH 专属源码在 DSH 窗口（创造模式）做最顺手，ZCode 侧的活同理——另一侧通过 git 历史与本文件「知情」，无需在场；跨 `content/` 的改动必须照第 1 节六步同步到另一侧并提醒重启。
...
DSH 侧相反：`link:` 安装者改完仓库、重启 `dsh web` 即生效，无需另外部署；
`~/.dsh/AGENTS.md` 自 2026-09-13 起只是指针文件，不要往里回填规则正文。
...
5. **规则文本多份手抄必漂移**——v0.4.0 之前 `collab-rules.md`（DSH）与 `global-agents.md`（ZCode）各写各的，ZCode 侧圆桌规则引用了不存在的角色名 `advisor`，圆桌调用不到任何东西；2026-09-13 又发现第三份：`~/.dsh/AGENTS.md` 还躺着 2026-09-10 的手工移植副本，而 DSH 会把用户全局说明注入每个会话，于是它圆桌指向不存在的三席名、还声称角色工具未定义，与插件规则正面对撞。→ 所以规则文本进单一来源（平台占位符），check-drift 有回归断言；**两个平台的全局 AGENTS.md 一律只放指针与环境说明，不放规则正文**——谁往里塞规则，谁就造出下一份漂移副本。
